...
var Lm = (function(Lm, $, undefined) {
	...
	function CalculateOrderTotalMilliLm(quantityQNT, priceMilliLm) {
		if (typeof quantityQNT != "object") {
			quantityQNT = new BigInteger(String(quantityQNT));
		}

		if (typeof priceMilliLm != "object") {
			priceMilliLm = new BigInteger(String(priceMilliLm));
		}

		var orderTotal = quantityQNT.multiply(priceMilliLm);

		return orderTotal.toString();
	}

	function CalculateOrderTotal(quantityQNT, priceMilliLm) {
		if (typeof quantityQNT != "object") {
			quantityQNT = new BigInteger(String(quantityQNT));
		}
		...
		return Lm.ConvertToLm(quantityQNT.multiply(priceMilliLm));
	}
	...
	function ConvertToLm(amount, returnAsObject) {
		...
		var fractionalPart = amount.mod(new BigInteger("1000")).toString(); //.replace(/0+$/, ""); //todo: check if equal to zero first

		amount = amount.divide(new BigInteger("1000"));
	...
	function ConvertToMilliLm(currency) {
