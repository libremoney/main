var Lm = (function(Lm, $, undefined) {
	...
	function EncryptMessage(secretPhrase, publicKey, message) {
		try {
			var privateKey = converters.hexStringToByteArray(lmCrypto.getPrivateKey(secretPhrase));
	...
	function SendMessageForm($modal) {
		var data = {
			...
			"feeLm": $.trim($("#send_message_fee").val()),
	...
	function SendMessageCompleteForm(response, data) {
		...
		if (Lm.CurrentPage == "messages") {
			var date = new Date(Date.UTC(2014, 04, 13, 16, 53, 20, 0)).getTime();
