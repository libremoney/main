var Lm = (function(Lm, $, undefined) {
			...
			if (response.errorCode) {
				...
				if (Lm.DownloadingBlockchain) {
					...
					$("#dashboard_message").addClass("alert-danger").removeClass("alert-success").html(
						...
						"<a href='#' data-toggle='modal' data-target='#send_money_modal'>send Nxt</a>, ...)"
					).show();
				} else {
				...
				$("#account_balance").html(Lm.FormatStyledAmount(response.unconfirmedBalanceNQT));
				$("#account_forged_balance").html(Lm.FormatStyledAmount(response.forgedBalanceNQT));
	...
	function UpdateAccountLeasingStatus() {
		...
		if (Lm.AccountInfo.effectiveBalanceNXT == 0) {
