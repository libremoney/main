			...
			if (response.publicKey) {
				callback({
					...
					"message": "The recipient account has a public key and a balance of " +
						Lm.FormatAmount(response.unconfirmedBalanceNQT, false, true) + "Lm.",
					"account": response
				});
			} else {
				if (response.errorCode) {
					if (response.errorCode == 4) {
						callback({
							...
							"message": "The recipient account is malformed, please adjust." +
								(!/^(NXT\-)/i.test(accountId) ? " If you want to type an alias, prepend it with the @ character." : ""),
							"account": null
						});
					...
					callback({
						...
						"message": "The recipient account does not have a public key, meaning it has never had an outgoing transaction. "+
							"The account has a balance of " + Lm.FormatAmount(response.unconfirmedBalanceNQT, false, true) + " Lm. "+
							"Please double check your recipient address before submitting.",
						"account": response
					});
				}
