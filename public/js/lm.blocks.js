		...
		if (Lm.Blocks.length < 10 && response.previousBlock) {
			...
			for (var i = 0; i < Lm.Blocks.length; i++) {
				...
				rows += "<tr><td><a href='#' data-block='" + String(block.height).escapeHTML() + "' data-blockid='" +
					...
					"<td>" + Lm.FormatAmount(block.totalAmountNQT) + " + " + Lm.FormatAmount(block.totalFeeNQT) + "</td>"+
	...
	function IncomingUpdateDashboardBlocks(newBlocks) {
		...
		for (var i = 0; i < newBlockCount; i++) {
			...
			rows += "<tr><td><a href='#' data-block='" + String(block.height).escapeHTML() +
				...
				Lm.FormatTimestamp(block.timestamp) + "</td><td>" + Lm.FormatAmount(block.totalAmountNQT) + " + " +
				Lm.FormatAmount(block.totalFeeNQT) + "</td><td>" + Lm.FormatAmount(block.numberOfTransactions) + "</td></tr>";
		}
	...
	function BlocksPageLoaded(blocks) {
		...
		for (var i = 0; i < blocks.length; i++) {
			...
			rows += "<tr><td><a href='#' data-block='" + String(block.height).escapeHTML() + "' data-blockid='" +
				...
				(block.numberOfTransactions > 0 ? " style='font-weight:bold'" : "") + ">" + String(block.height).escapeHTML() + "</a></td>"+
				"<td>" + Lm.FormatTimestamp(block.timestamp) + "</td>"+
				"<td>" + Lm.FormatAmount(block.totalAmountNQT) + "</td>"+
				"<td>" + Lm.FormatAmount(block.totalFeeNQT) + "</td>"+
		...
		averageFee = Lm.ConvertToNQT(averageFee);
		averageAmount = Lm.ConvertToNQT(averageAmount);
		...
		if (Lm.BlocksPageType == "forged_blocks") {
			...
			$("#forged_fees_total").html(Lm.FormatStyledAmount(Lm.AccountInfo.forgedBalanceNQT)).removeClass("loading_dots");
		} else {
			if (time == 0) {
